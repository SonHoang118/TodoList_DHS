...
const Header: React.FC<HeaderProps> = ({ goToToday, moveWeek, headerTitle, onCurrentUserChange }) => {
    ...
    <header className="border-b border-[#e0e0e0] bg-[#f1f3f4]">
      <div className="mx-auto flex h-18 max-w-400 items-center gap-4 px-3 md:px-6">
        <div className="relative hidden md:flex">
          <div
            className="flex items-center gap-3 rounded-full bg-white px-3 py-2 shadow-sm cursor-pointer min-w-[140px]"
          ...
          >
            ...
            <span className="text-sm font-medium truncate max-w-[80px]">{currentUser ? currentUser.fullName : ""}</span>
            <svg className={`ml-1 w-4 h-4 transition-transform ${dropdownOpen ? "rotate-180" : "rotate-0"}`} fill="none" stroke="#6b7280" strokeWidth="2" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7"/></svg>
          </div>
          {dropdownOpen && (
